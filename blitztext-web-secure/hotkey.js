...
export function loadHotkey() {
  try {
    ...
    if (raw) {
      ...
      return {
        code:            typeof p.code === 'string'  ? p.code            : DEFAULT_HOTKEY.code,
        ctrlKey:         typeof p.ctrlKey === 'boolean' ? p.ctrlKey      : DEFAULT_HOTKEY.ctrlKey,
        shiftKey:        typeof p.shiftKey === 'boolean' ? p.shiftKey    : DEFAULT_HOTKEY.shiftKey,
        altKey:          typeof p.altKey === 'boolean' ? p.altKey        : DEFAULT_HOTKEY.altKey,
        requireLeftCtrl: typeof p.requireLeftCtrl === 'boolean' ? p.requireLeftCtrl : DEFAULT_HOTKEY.requireLeftCtrl,
      };
    }
  } catch (_) {}
